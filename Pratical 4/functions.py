...
class Rosenbrock(ScalarFunction):
    ...
    def jacobian(self, x):
        ...
        derivative[0] = 100 * 2 * (x2 - x1**2) * (-2 * x1) - 2 * (1 - x1)
        derivative[1] = 100 * 2 * (x2 - x1**2)
    ...
    def hessian(self, x):
        ...
        second_derivative[0][0] = 200 * ((-2 * x1) ** 2 - 2 * (x2 - x1**2)) + 2
        second_derivative[0][1] = 200 * (-2 * x1)
        second_derivative[1][0] = 200 * (-2 * x1)
...
class LogisticRegressionNLL(ScalarFunction):
    ...
    def jacobian(self, phi):
        ...
        tmp = sigmoid(phi.T @ self.x)
        derivative = -(self.x @ (tmp - self.y).T).reshape(-1, 1)
    ...
    def hessian(self, phi):
        ...
        tmp = sigmoid(phi.T @ self.x)
        tmp = tmp.T @ (1 - tmp)
        second_derivative = -(self.x @ tmp @ self.x.T)
